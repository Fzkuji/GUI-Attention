"""
Saccade Foveation Training (v4): LoRA + ActionHead, 2-round teacher forcing.

No gui_aima dependency. Uses transformers Qwen2.5-VL + peft LoRA + ActionHead.

Each sample trains 2 rounds:
  Round 0: [low-res full image] [instruction] [anchor_0]
    → forward → LLM hidden states → action head
    → labels: binary overlap of low-res patches with GT bbox
    → loss_0 = KL_div(predicted, normalised(labels))

  Round 1: [low-res full image] [instruction] [anchor_0] [high-res crop] [anchor_1]
    → forward → LLM hidden states → action head (mask overlapping low-res patches)
    → labels: all-0 (low-res) + binary overlap of high-res patches with local GT
    → loss_1 = KL_div(predicted, normalised(labels))

  Total loss = loss_0 + loss_1
...
class SaccadeTrainer:
    ...
    # -- 2-round teacher forcing forward ------------------------------------

    def _forward_sample(self, sample):
        """2-round teacher forcing: low-res → low-res + high-res crop.
        ...
        last_hs0 = outputs0.hidden_states[-1]  # (1, seq, d)

        vis_hidden0, vis_ranges0 = extract_visual_hidden_states(last_hs0, inp0["input_ids"], img_tok)
        anchor0 = extract_anchor_hidden_states(last_hs0, inp0["input_ids"], pp_id, n=0)

        if vis_hidden0 is not None and anchor0 is not None:
            grid_dims0 = self.builder.get_image_grid_dims(inp0["image_grid_thw"], merge)
            nh0, nw0 = grid_dims0[0]

            # Binary labels for round 0
            labels0 = compute_binary_labels(nh0, nw0, bbox_gt).to(device).unsqueeze(0)  # (1, n_vis)
            ...
            with torch.no_grad():
                attn_1d0 = attn0.squeeze(0)
                _, local_idx0 = identify_attended_image(attn_1d0, vis_ranges0)
                lx0, ly0 = token_to_spatial(local_idx0, nw0, nh0)
                round_preds.append((lx0, ly0))

        # ===== Round 1: low-res + high-res crop =====
        # Crop center = GT center + random jitter
        jitter_x = random.uniform(-self.sa.crop_jitter, self.sa.crop_jitter)
        jitter_y = random.uniform(-self.sa.crop_jitter, self.sa.crop_jitter)
        crop_cx = max(0.0, min(1.0, gt_cx + jitter_x))
        crop_cy = max(0.0, min(1.0, gt_cy + jitter_y))

        cropped, crop_bbox = crop_image(img, crop_cx, crop_cy, self.sa.crop_ratio)
        try:
            r1_inputs, cur_text, cur_images = self.builder.extend_with_crop(
                cur_text, cur_images, cropped, crop_bbox,
            )
        except Exception:
            return total_loss, n_valid, round_preds

        inp1 = {k: v.to(device) for k, v in r1_inputs.items()}
        ...
        last_hs1 = outputs1.hidden_states[-1]

        vis_hidden1, vis_ranges1 = extract_visual_hidden_states(last_hs1, inp1["input_ids"], img_tok)
        anchor1 = extract_anchor_hidden_states(last_hs1, inp1["input_ids"], pp_id, n=1)

        if vis_hidden1 is not None and anchor1 is not None and len(vis_ranges1) >= 2:
            grid_dims1 = self.builder.get_image_grid_dims(inp1["image_grid_thw"], merge)
            nh_low, nw_low = grid_dims1[0]
            nh_high, nw_high = grid_dims1[1]

            n_low = vis_ranges1[0][1]
            n_high = vis_ranges1[1][1]
            n_total = n_low + n_high

            # Labels: all-0 for low-res, binary overlap for high-res in local coords
            ...
            if cbw > 0 and cbh > 0:
                ...
                high_labels = torch.zeros(n_high)

            full_labels = torch.zeros(1, n_total, device=device)
            full_labels[0, n_low:n_low + n_high] = high_labels.to(device)
            ...
            attn1, loss1 = self.model.action_head(vis_hidden1, anchor1, labels=full_labels, mask=full_mask)
            ...
            # Prediction for metrics
            with torch.no_grad():
                attn_1d1 = attn1.squeeze(0)
                img_idx1, local_idx1 = identify_attended_image(attn_1d1, vis_ranges1)
                info1 = self.builder.image_infos[img_idx1]
                if img_idx1 < len(grid_dims1):
                    nh_a, nw_a = grid_dims1[img_idx1]
                    lx1, ly1 = token_to_spatial(local_idx1, nw_a, nh_a)
                    bx1, by1, bx2, by2 = info1.global_bbox
                    ox1 = bx1 + lx1 * (bx2 - bx1)
                    oy1 = by1 + ly1 * (by2 - by1)
                    round_preds.append((ox1, oy1))
    ...
    def train(self):
        ...
        if self.rank == 0:
            ...
            print(f"  crop_ratio={self.sa.crop_ratio}  crop_jitter={self.sa.crop_jitter}")
